...
namespace Nito.Comparers.Util
{
    ...
    internal abstract class SourceEqualityComparerBase<T, TSource> : EqualityComparerBase<T>
    {
        ...
        /// The source comparer.
        /// </summary>
        protected readonly IEqualityComparer<TSource> Source;

        /// <summary>
        /// Initializes a new instance of the <see cref="SourceEqualityComparerBase{T,TSource}"/> class.
        /// </summary>
        /// <param name="source">The source comparer. If this is <c>null</c>, the default comparer is used.</param>
        /// <param name="specialNullHandling">A value indicating whether <c>null</c> values are passed to <see cref="EqualityComparerBase{T}.DoGetHashCode"/> and <see cref="EqualityComparerBase{T}.DoEquals"/>. If <c>false</c>, then <c>null</c> values are considered not equal to any non-<c>null</c> values and are not passed to <see cref="EqualityComparerBase{T}.DoGetHashCode"/> nor <see cref="EqualityComparerBase{T}.DoEquals"/>. This value is ignored if <typeparamref name="T"/> is a non-nullable type.</param>
        protected SourceEqualityComparerBase(IEqualityComparer<TSource>? source, bool specialNullHandling)
            : base(specialNullHandling)
        {
            Source = EqualityComparerHelpers.NormalizeDefault(source);
        }
    }
}
